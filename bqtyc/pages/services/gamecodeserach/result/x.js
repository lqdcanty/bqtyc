...
//  pageNumber: thePage.data.pageNumber, pageSize: thePage.data.pageSize, keyword: ''
  getData:function(param){
    ...
    app.doPost('https://www.ry365.com.cn/gamecode/plist', { pageNumber: thePage.data.pageNumber, pageSize: thePage.data.pageSize, keyword: thePage.data.keyword }, null, function (data) {
      console.log(data.result.list)
      var  worksList;
      if (thePage.data.worksList==null){
        worksList =data.result.list.list;
      }else{
        worksList = thePage.data.worksList.concat(data.result.list.list)
      }
      if (thePage.data.pageNumber == data.result.list.totalPage || thePage.data.pageNumber > data.result.list.totalPage){
        thePage.setData({
          worksList: worksList,
          totalPage: data.result.list.totalPage,
          flage: true
        });
      }else{
        thePage.setData({
  ...
  /**
   * 生命周期函数--监听页面显示isbnNoisbnNoisbnNoisbnNoisbnNoisbnNoisbnNoisbnNoisbnNoisbnNoisbnNoisbnNois
   * is
   * iiiifsdfs sfdd fsdf
   */
  onShow: function () {
