...
Page({
  ...
  onLoad: function (options) {
    ...
    let title = thePage.data.navTitle;
    if (options.citiaoClass==2){
      title='删除许可使用'
    } else if (options.citiaoClass == 3){
      title = '删除权利转让'
    } else if (options.citiaoClass == 4) {
      title = '删除权利质押'
    }
    thePage.setData({
      navTitle: title
  ...
  getData(){
    ...
    app.doPost('/citiao/get', { token: app.globalData.userInfo.authToken, citiaoCode: thePage.data.citiaoCode, tableType: thePage.data.tableType, timestamp: (new Date()).valueOf() }, null, function (data) {
      ...
      if (data.result.model.citiao.deleteApplyItem) {
        _citiao.attathRemark = _citiao.deleteApplyItem.attachRemark;
        _citiao.contactName = _citiao.deleteApplyItem.contactName;
        _citiao.contactEmail = _citiao.deleteApplyItem.contactEmail;
        _citiao.contactPhone = _citiao.deleteApplyItem.contactPhone;
        thePage.setData({
          citiao: _citiao,
